...
export default function ConnectCardBanner() {
  const { user, cardConnected, markCardConnected } = useAuth();
  const { removeNotification } = useNotifications();
  const [status, setStatus] = useState<Status>('idle');
  const [dismissed, setDismissed] = useState(false);

  // Show only for retailers whose card is confirmed NOT connected.
  // null means still loading — hide until we know for sure.
  if (!user || user.role !== 'retailer' || cardConnected !== false || dismissed) {
    return null;
  }
  ...
  const verifyAndSave = (reference: string) => {
    ...
    fetch('/api/paystack/verify', {
      ...
      .then((r) => { if (!r.ok) throw new Error(); return r.json() as Promise<{ authorization: Record<string, unknown> }>; })
      .then(({ authorization }) => {
        if (!user?.uid) return;
        ...
        markCardConnected();          // update AuthContext state immediately
        removeNotification('card-required').catch(() => {}); // best-effort badge cleanup
        setStatus('success');
      })
    ...
    return (
      <div className="shrink-0 flex items-center gap-3 px-4 py-2.5 bg-green-500/10 border-b border-green-500/20">
        ...
        <p className="flex-1 text-sm font-medium text-green-400">
          Card connected! Wholesalers can now extend credit to you.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="shrink-0 flex items-center gap-3 px-4 py-2.5 bg-amber-500/10 border-b border-amber-500/20">
      ...
      <p className="flex-1 text-xs sm:text-sm text-foreground/80 leading-snug">
        ...
        connect a repayment card so wholesalers can extend credit to you.
      </p>
      ...
      <button
        ...
        className="shrink-0 h-8 px-3 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors disabled:opacity-60 disabled:cursor-not-allowed whitespace-nowrap"
      >
        {status === 'opening' ? (
          ...
          <><CreditCard size={11} /> Connect Card</>
        )}
      </button>
