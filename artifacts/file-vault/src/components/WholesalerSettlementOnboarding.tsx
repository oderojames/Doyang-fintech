...
import { CreditCard, Loader2, CheckCircle2, AlertCircle, ArrowLeft, ChevronDown } from 'lucide-react';
...
type Step = 'bank' | 'account' | 'confirm' | 'success';
...
export default function WholesalerSettlementOnboarding({ onComplete, fullscreen = false }: Props) {
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      setDoc(doc(db, 'users', user.uid, 'notifications', 'settlement-required'), {
        ...
        body: 'Add your bank account details so Doyang can settle loan repayments to you.',
  ...
  const steps: Step[] = ['bank', 'account', 'confirm'];
      ...
      <div className="relative w-full max-w-lg">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Connect your bank account to receive loan repayment settlements.
          </p>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl shadow-2xl overflow-hidden">
          ...
          {step === 'confirm' && (
            <div className="p-6 space-y-5">
              <div className="flex items-center gap-2.5">
                <button onClick={() => setStep('account')} className="text-muted-foreground hover:text-foreground transition-colors">
                  <ArrowLeft size={16} />
                </button>
                <div>
                  <h3 className="font-semibold text-foreground">Confirm details</h3>
                  <p className="text-xs text-muted-foreground">Review before creating your settlement account</p>
                </div>
              </div>
              ...
              >
                {submitStatus === 'loading'
                  ? <><Loader2 size={14} className="animate-spin" />Creating settlement account…</>
            ...
            <div className="p-8 flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                  Your settlement account at <span className="font-semibold text-foreground">{selectedBank?.name}</span> is connected. Repayments will be settled directly to your account.
                </p>
              </div>
        ...
        {step !== 'success' && (
          <p className="text-[11px] text-muted-foreground text-center mt-4 leading-relaxed">
            Bank details are sent securely to Paystack. Doyang never stores your account credentials.
          </p>
        )}
      </div>
    </div>
